...
using ..DatasetVel: load_dataset, make_batches
using ..UNetPsi: build_unet   # gleiches U-Net, nur andere Kanalzahlen
...
"""
    train_unet_vel(; data_dir, ...)

Trainiert ein U-Net mit 1 Input-Kanal (Maske) und 2 Output-Kanälen (Vx,Vz).
"""
function train_unet_vel(; data_dir::String="data_vel",
                         epochs::Int=10,
                         batch_size::Int=4,
                         lr::Float64=1e-4,
                         rng = Random.default_rng(),
                         save_path::String="unet_vel.bson",
                         use_gpu::Union{Nothing,Bool}=nothing)

    move, devsym = _select_mover(use_gpu)
    @info "Trainiere Ansatz 1 (v) auf Gerät: $(devsym == :gpu ? "GPU" : "CPU")"
    ...
    # 1 Input-Kanal (Maske), 2 Output-Kanäle (Vx, Vz)
    model = build_unet(1, 2)
    ...
    @info "Training Ansatz 1 abgeschlossen. Modell gespeichert nach $save_path"
    return model
end
