...
\cite{ribeiro_deepcfd_nodate,thuerey_deep_2020}. This choice injects physical
...
advocated in related work \cite{richter-powell_neural_nodate}.
...
contrasts is essential \cite{thieulot_choice_2022}.  
...
sedimentation studies \cite{fortes_nonlinear_1987,ladd_numerical_1994} and
...
is standard in two-dimensional incompressible low-Reynolds-number flow theory
\cite{happel_low_1991}. In the present workflow, this Poisson solve is used
...
All approaches use the same four-channel input representation on the
...
geometry-aware learning frameworks \cite{thuerey_deep_2020,oldenburg_geometry_2022}.
...
$(256,256,4)$. In physical terms, the input encodes inclusion geometry and
domain position; in ML terms, it provides both high-frequency (mask) and
low-frequency (distance and coordinate) information to support multiscale
feature learning.
...
four-channel input of shape $(256,256,4)$ and produce a single-channel output
...
\cite{navab_u-net_2015} and common adaptations for laminar flow-field
prediction on Cartesian grids \cite{thuerey_deep_2020,chen_u-net_nodate}.
...
\paragraph{Lifting.}
A pointwise linear layer maps the four input channels into a
...
training data and the physically structured output representation. Dropout is
not used, as preliminary experiments indicated that on-the-fly sampling of
random crystal configurations provides sufficient regularization.
...
The dataset is conceptually divided into training, validation, and test
splits. Training and validation data are generated from randomized crystal
configurations, whereas the test split consists of a fixed set of geometries
held constant throughout the study. This design ensures that model selection
is performed on distributions similar to training, while the reported test
performance reflects generalization to a reproducible benchmark.
...
Model parameters are optimized using AdamW, which combines adaptive learning
rates with decoupled weight decay regularization. A fixed learning rate is
used within each run, without learning rate scheduling, to keep experiments
comparable. After each epoch, model checkpoints are stored in a CPU-compatible
...
surrogates \cite{luo_cfdbench_2023}. The same evaluation framework is applied
