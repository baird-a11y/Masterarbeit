...
\begin{itemize}
    \item \textbf{Fixed-radius baseline.} The best-performing models from Experiment~1 — FNO Exp-1.4 (bs\,16, lr~$=5\times10^{-4}$) and the best U-Net configuration — are evaluated on single-crystal samples ($n=1$) with unseen radii, both substantially smaller ($r = 0.005$) and substantially larger ($r = 0.5$) than the training radius $r = 0.05$. This quantifies the size-generalization error for models that have had no exposure to size variability, and assesses whether either architecture is inherently more robust to geometric scale changes.

    \item \textbf{Variable-radius training.} A new FNO is trained on single-crystal samples ($n = 1$) with three discrete radii: $r \in \{0.02,\, 0.05,\, 0.08\}$ (1\,000 samples each, i.e., 3\,000 training samples total). This condition is evaluated for the FNO only: since the U-Net showed substantially larger prediction errors in Experiments~2 and~3, the more informative question is whether the stronger FNO architecture can leverage explicit size variability in training to improve generalization across the size axis. The radius channel of the input representation explicitly encodes the crystal size at each grid point, enabling the network to condition its prediction on the local crystal geometry.
\end{itemize}
...
A detailed analysis of the results is provided in Section~\ref{ssec:results:exp4}.
