...
function train_model(model, train_data, val_data; epochs, learning_rate)
    ...
    for epoch in 1:epochs
        ...
        for (x_batch, y_batch) in train_data
            # Normalize masks
            # Konvertiere beide Batches zu Float32
...
function train_model_cuda(model, train_data, val_data; epochs, learning_rate)
    ...
    Args:
        model: The U-Net model instance to be trained.
        train_data: Training dataset (batches) on GPU.
        val_data: Validation dataset (batches) on GPU.
        epochs: Number of training epochs.
        learning_rate: Learning rate for the optimizer.
    ...
    for epoch in 1:epochs
        ...
        # Training loop
        for (x_batch, y_batch) in train_data
            # Normalize masks (auf der GPU bleiben)
