...
(batch size~16, learning rate $10^{-3}$, 50~epochs) and evaluates each on all training
crystal counts as well as on unseen higher counts up to $n = 25$.
The hyperparameter choice of lr~$= 10^{-3}$ and bs~$= 16$ is motivated by the Experiment~1
FNO results, where this configuration achieved the best single-crystal performance ($1.85\%$).
For the U-Net, Experiment~1 showed that lr~$= 10^{-4}$ yields the lowest validation error
on the single-crystal task; however, with only 50~epochs available, a lower learning rate
risks even slower convergence on the harder multi-crystal task. The lr~$= 10^{-3}$ adopted
here is a practical compromise that prioritizes training speed at the cost of some oscillation.
...
(batch size~16, learning rate $10^{-3}$, 50~epochs) and evaluates each on substantially
...
but remains significant: at $n = 50$, the FNO's $14.2\%$ error is about half the U-Net's
$31.4\%$. The velocity errors tell a similar story: the FNO's ${\sim}16\%$ at $n=50$
