...
Liest alle .jld2-Dateien im Verzeichnis `dir` und baut ein ResidualDataset.
...
function load_dataset(dir::String; max_samples::Union{Int,Nothing}=nothing)
    ...
    @info "Residual-Dataset geladen: $(length(all_files)) Samples aus $dir"
    return ResidualDataset(all_files)
end

"""
    get_sample(ds, idx)

Lädt ein einzelnes Residual-Sample:
- input         :: (nx, nz, 1) Float32
- residual_norm :: (nx, nz, 1) Float32
"""
function get_sample(ds::ResidualDataset, idx::Int)
    file = ds.files[idx]
    input = nothing
    residual_norm = nothing
    @load file input residual_norm

    x = Float32.(input)          # (nx, nz, 1)
    r = Float32.(residual_norm)  # (nx, nz)

    nx, nz = size(r)
    y = reshape(r, nx, nz, 1)    # (nx, nz, 1)
...
Batches:
- x_batch: (nx, nz, 1, B)
- y_batch: (nx, nz, 1, B)  (Residual)
"""
function make_batches(ds::ResidualDataset, batch_size::Int; rng=Random.default_rng())
    N = length(ds.files)
    idxs = collect(1:N)
