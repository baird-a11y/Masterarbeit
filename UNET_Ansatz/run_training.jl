...
α_grad_target   = 0.1       # Zielwert für Gradient-Loss-Koeffizient
warmup_epochs   = 5         # Warmup-Epochen für α_grad
α_bnd           = 0.01      # Rand-Loss-Koeffizient (konstant)
...
for i in eachindex(ARGS)
    ...
    if ARGS[i] == "--alpha_grad"    && i < length(ARGS); global α_grad_target = parse(Float64, ARGS[i+1]); end
    if ARGS[i] == "--warmup_epochs" && i < length(ARGS); global warmup_epochs = parse(Int,     ARGS[i+1]); end
    if ARGS[i] == "--alpha_bnd"     && i < length(ARGS); global α_bnd         = parse(Float64, ARGS[i+1]); end
...
Training-Konfiguration (U-Net):
  ...
  α_grad_target: $α_grad_target  (Warmup: $warmup_epochs Epochen)
  α_bnd:         $α_bnd
"""
