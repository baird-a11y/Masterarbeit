# Datei: train.jl
# Training-Pipeline für ψ-FNO
# Vollständig getrennt von Model/Dataset/Losses.
#
# Funktion              Was sie tut
# get_device            CPU/GPU Auswahl
# grad_alpha            Warmup-Schedule für Grad-Loss α
# train_step!           Ein Batch: forward → loss → backward → update
# train_epoch!          Alle Batches einer Epoche
# validate!             Validation ohne Gradienten
# append_history!       Zeile an CSV anhängen
# save_checkpoint       Modell + Optimizer + Metriken speichern
# train!                Orchestrator: Epochen-Loop mit Logging/Checkpoint
...
"""Berechnet die globale L2-Norm aller Gradienten (Flux 0.15+ gradient tree)."""
...
"""
    save_checkpoint(path, model, opt_state, epoch, metrics)
...
function train!(; model,
    ...
    for epoch in 1:epochs
        ...
        row = (
            ...
            alpha     = α_grad,
            val_mse   = get(val_metrics, :loss_mse, NaN),
            val_rel_l2 = get(val_metrics, :rel_l2, NaN),
            val_max_error = get(val_metrics, :max_error, NaN),
