...
using the AdamW optimizer, which combines adaptive learning-rate updates with
...
$\psi_{\mathrm{norm}}$. Unless otherwise noted, a fixed weight decay is used
within AdamW to provide mild regularization across runs.
...
$\psi_{\mathrm{norm}}$ from a 4-channel input consisting of
(i) the crystal mask,
(ii) a signed distance field (SDF),
(iii) normalized $x$ coordinates, and
(iv) normalized $z$ coordinates.
