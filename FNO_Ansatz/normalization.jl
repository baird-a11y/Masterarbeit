...
# ψ-Normalisierung: Gauge-Fix (Offset) → Skalierung → Normalize/Denormalize
# Funktion	                                Was sie tut
# NormMeta	                                Struct mit offset, scale, strategy, p_mean, maxabs, std_val
# gauge_fix_psi	                            Offset fixieren: :fluid_mean (mit phase), :global_mean, :corner → (ψ_fixed, offset)
# compute_scale	                            Skalenfaktor: :powmean (mittl. 10er-Potenz), :maxabs, :std → (scale, NormMeta)
# normalize_psi	                            ψ_norm = ψ_fixed * scale
# denormalize_psi	                        ψ = ψ_norm / scale + offset (auch mit NormMeta-Overload)
# normalize_sample!	                        One-call: gauge-fix → scale → normalize, gibt erweitertes NamedTuple zurück
# sanity_norm_checks	                    Diagnostik + Warnungen (NaN/Inf, extreme Werte, Roundtrip-Fehler)
...
# =============================================================================
# Schritt N4 – NormMeta Struct (definiert zuerst, da andere Funktionen es nutzen)
# =============================================================================
...
# =============================================================================
# Schritt N1 – Gauge-Fix (Offset-Invarianz)
# =============================================================================
...
function gauge_fix_psi(ψ::AbstractMatrix;
    ...
    if mode == :fluid_mean
        ...
        if n_fluid == 0
            @warn "Keine Fluid-Zellen gefunden (alle Zellen == phase_crystal=$phase_crystal), Fallback auf :global_mean"
...
# =============================================================================
# Schritt N2 – Skalenfaktor bestimmen
# =============================================================================
...
Gibt `(scale, meta::NormMeta)` zurück (offset im Meta wird auf 0 gesetzt,
da der Gauge-Fix separat passiert).
"""
function compute_scale(ψ_fixed::AbstractMatrix;
    ...
    elseif strategy == :powmean
        ...
        # Nur nicht-null Werte verwenden
...
# =============================================================================
# Schritt N3 – Normalisieren & Denormalisieren
# =============================================================================
