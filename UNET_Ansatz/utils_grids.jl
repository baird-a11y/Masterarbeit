...
# Grid-Utilities für finite Differenzen, Divergenz, Vorticity, etc.
# Analog zum FNO_Ansatz
...
# - Default BC for derivatives: :onesided (robust, works with Dirichlet ψ=0)
# =============================================================================
...
Return (is_uniform_x, is_uniform_z).
"""
function check_uniform_grid(x_vec_1D::AbstractVector, z_vec_1D::AbstractVector;
                            rtol=1e-10, atol=0.0)
    dxs = diff(x_vec_1D)
    dzs = diff(z_vec_1D)
...
- range=:unit -> [0, 1]
"""
function normalize_coords(x_vec_1D::AbstractVector, z_vec_1D::AbstractVector;
                          range::Symbol = :pm1)
...
bc options: :onesided (forward/backward at boundaries, central inside)
"""
...
Compute ∇·V = ∂Vx/∂x + ∂Vz/∂z.
"""
function divergence(Vx::AbstractMatrix, Vz::AbstractMatrix, dx::Real, dz::Real;
                    bc::Symbol = :onesided)
...
Compute vorticity ω = ∂Vz/∂x - ∂Vx/∂z.
"""
function vorticity(Vx::AbstractMatrix, Vz::AbstractMatrix, dx::Real, dz::Real;
                   bc::Symbol = :onesided)
    @assert size(Vx) == size(Vz)
...
Boolean mask true on interior cells (excluding boundary band of `width`).
"""
