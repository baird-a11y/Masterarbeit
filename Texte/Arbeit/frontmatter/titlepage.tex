% PROFESSIONAL TITLE PAGE
\begin{titlepage}
    \centering
    
    % University Logo (if available)
    % \includegraphics[width=0.3\textwidth]{logo_uni_mainz.png}\\[1cm]
    
    % University Information
    {\large \textbf{Johannes Gutenberg University Mainz}}\\[0.5cm]
    {\large Department 08 -- Physics, Mathematics and Computer Science}\\[0.5cm]
    {\large Institute of Geosciences}\\[2cm]
    
    
    % Thesis Title
    {\huge \textbf{Data-Driven Stokes Flow Prediction in Multi-Crystal Sedimentation: }}\\[0.5cm]
    {\LARGE \textbf{A Systematic Comparison of Spatial and Spectral Neural Surrogate Architectures}}\\[1.5cm]
    
    % Type of Thesis
    {\Large \textbf{Master's Thesis}}\\[0.5cm]
    submitted in partial fulfillment of the requirements (bitti bitti)\\[0.3cm]
    for the degree of\\[0.3cm]
    {\large \textbf{Master of Science (M.Sc.)}}\\[0.3cm]
    in\\[0.3cm]
    {\large \textbf{Computational Sciences}}\\[2cm]
    
    % Author
    submitted by\\[0.5cm]
    {\large \textbf{Paul Baselt}}\\[0.3cm]
    Matriculation Number: 2706608\\[0.3cm]
    born in Berlin\\[1.5cm]
    
    % Supervisors
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \textbf{First Supervisor:}\\
        Univ.-Prof. Dr. Boris J.\ P.\ Kaus\\
        Institute of Geosciences\\
        Johannes Gutenberg-Universität Mainz
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \textbf{Second Supervisor:}\\
        Prof. Dr. Michael Wand\\
        Institute of Computer Science\\
        Johannes Gutenberg-Universität Mainz
    \end{minipage}\\[2cm]
    
    % Date
    \vfill
    {\large Mainz, \today}
    
\end{titlepage}
